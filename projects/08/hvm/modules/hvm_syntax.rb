...
assert_equal syntax.type, :C_PUSH
...
assert_equal syntax.parse_args('push local 0')[:arg1], 'local'
assert_equal syntax.parse_args('push local 0')[:arg2], '0'
assert_equal syntax.parse_args('pop local 0'), {}

# Test one-argument syntax
syntax = HvmSyntax.new(/goto (\w+)/, :C_GOTO)
assert syntax.applies_to?('goto label')
assert_equal syntax.parse_args('goto label')[:arg1], 'label'
assert_equal syntax.parse_args('goto label')[:arg2], nil

# Test no-argument syntax
syntax = HvmSyntax.new(/eq/, :C_ARITHMETIC)
assert !syntax.applies_to?('equal')
assert_equal syntax.parse_args('eq')[:arg1], 'eq'
